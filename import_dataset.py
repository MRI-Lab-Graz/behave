...
def convert_excel_to_json_updated(excel_file, output_json_file, anonymize=False):
    ...
    # Skip rows with placeholder 'Unknown Item' or missing values if necessary
        ...
        #print(excel_file)
        # Fetch levels safely, default to None if missing
        levels = row.get("levels", None)
        print(excel_file)

        # Ensure 'levels' is valid before conversion
        if pd.notna(levels) and int(levels) == 0 and pd.notna(row.get("leveldescription", None)):
            entry["Units"] = row["leveldescription"]    
